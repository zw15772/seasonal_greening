...
class Build_early_dataframe:
    ...
    def add_CO2_to_df(self, df):
        fdir = results_root + 'extraction/extraction_original_val/extraction_pre_early_static/20%_Pre_CO2_extraction/'
        for f in (os.listdir(fdir)):
            # print()
            CO2_dic = T.load_npy(fdir + f)
            f_name = 'early_'+f.split('.')[0]
            CO2_list = []
            for i, row in tqdm(df.iterrows(), total=len(df)):
                year = row['year']
                # pix = row.pix
    ...
    def add_during_CO2_to_df(self, df):  # add 14年的CO2 during early, peak and late
        f = results_root + 'extraction/extraction_original_val/extraction_during_early_growing_season_static/during_early_CO2.npy'
    ...
    def drop_field_df(self,df):
        df = df.drop(columns=['CSIF_par_trend_late'])
        return df
...
class Build_peak_dataframe:
    ...
    def add_CSIF_change_rate_trend_to_df(self, df):
        f = results_root + 'change_trend/peak_change_trend.npy'
        trend_array = np.load(f)
        trend_dic = DIC_and_TIF().spatial_arr_to_dic(trend_array)
        trend_list = []
        for i, row in tqdm(df.iterrows(), total=len(df)):
            ...
            trend_list.append(vals)
            # landcover_list.append(vals)
        df['%CSIF_par_trend_peak'] = trend_list
        return df
    ...
    def add_CO2_to_df(self, df):
        fdir = results_root + 'extraction/extraction_original_val/extraction_pre_peak_static/20%_Pre_CO2_extraction/'
        for f in (os.listdir(fdir)):
            # print()
            CO2_dic = T.load_npy(fdir + f)
    ...
    def extraction_drivers_facilitators(self, df):
        ...
        for i in range(len([drivers,facilitators])):
            ...
            #
    def drop_field_df(self,df):
        df = df.drop(columns=['CSIF_par_p_value_peak'])
        return df
