...
class Tools:
    ...
    def load_npy_dir(self,fdir,condition=''):
        dic = {}
        for f in tqdm(T.listdir(fdir),desc='loading '+fdir):
    ...
    def lonlat_to_address(self,lon, lat):
        ak = "mziulWyNDGkBdDnFxWDTvELlMSun8Obt"  # ????×?????????
...
class DIC_and_TIF:
    ...
    def __init__(self,tif_template=None):
        ...
        if tif_template == None:
            self.tif_template = '/Volumes/SSD_sumsang/project_greening/Data/SPEI/SPEI12/200101.tif'
        else:
...
class KDE_plot:
    ...
    def plot_scatter(self, val1, val2,
        ...
        if len(val1) > max_n:
            ...
            val_range_index = random.sample(val_range_index, max_n)  # ??val?????ú????n???????????????ì????????·¨
            new_val1 = []
            new_val2 = []
...
class Pre_Process:
    ...
    def data_transform_with_date_list(self, fdir, outdir,date_list):
        ...
        outdir = outdir + '/'
        # ????????×???????×é
        template_f = os.path.join(fdir,T.listdir(fdir)[0])
    ...
    def detrend(self,fdir,outdir):
        Tools().mk_dir(outdir)
        for f in tqdm(T.listdir(fdir),desc='detrend...'):
            dic = Tools().load_npy(fdir + f)
